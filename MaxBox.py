...
def BoxCons(inputs, hull):
    ...
    Pos = np.array(
        [
            [hull.delta_bow(hull.Dd), hull.delta_stern(hull.Dd)],
            [
                hull.delta_bow(hull.Dd - inputs[2]),
                hull.delta_stern(hull.Dd - inputs[2]),
            ],
        ]
    )
        ...
        for j in range(0, 2):
            ...
            if inputs[i] > Pos[j, 1]:
                if inputs[i] > hull.stern_profile(depths[j]):
                    ...
                    y = hull.halfBeam_Stern(
                        [inputs[i]], hull.solve_waterline_stern(depths[j])
                    )
            ...
            # next check if the position is in the bow profile
                ...
                if inputs[i] < hull.bow_profile(depths[j]):
                    ...
                    y = hull.halfBeam_Bow(
                        [inputs[i]], hull.solve_waterline_bow(depths[j])
                    )
...
def PC_BoxCon(points, box_inputs, Dd):
    ...
    in_box = np.logical_and(
        np.all(points >= box_min, axis=1), np.all(points <= box_max, axis=1)
    )
...
class BoxOpt(ElementwiseProblem):
    ...
    def __init__(self, x):
        ...
        self.PC = self.hull.gen_MeshGridPointCloud(
            NUM_WL=201, PointsPerLOA=501, Z=[], X=[], bit_GridOrList=0
        )
    ...
    def _evaluate(self, inputs, out, *args, **kwargs):
        ...
        out["G"] = np.concatenate(
            (BoxCons(inputs, self.hull), [PC_BoxCon(self.PC, inputs, self.hull.Dd)])
        )
