...
def SurfArea(X, Z, Y):
    ...
    for i in range(1, len(X) - 1):
        for j in range(1, len(Z) - 1):
            ...
            L1 = C[0] * np.sqrt(
                (X[i] - X[i - 1]) ** 2.0 + (Y[i, j] - Y[i - 1, j]) ** 2.0
            ) + C[1] * np.sqrt(
                (X[i] - X[i + 1]) ** 2.0 + (Y[i, j] - Y[i + 1, j]) ** 2.0
            )

            L2 = C[2] * np.sqrt(
                (Z[j] - Z[j - 1]) ** 2.0 + (Y[i, j] - Y[i, j - 1]) ** 2.0
            ) + C[3] * np.sqrt(
                (Z[j] - Z[j + 1]) ** 2.0 + (Y[i, j] - Y[i, j + 1]) ** 2.0
            )
...
def GaussianCurvature(x):
    ...
    for i in range(1, len(X) - 1):
        for j in range(1, len(Z) - 1):
            ...
            K1 = ((Y[i - 1, j] - 2 * Y[i, j] + Y[i + 1, j]) / hx**2.0) / (
                (1 + ((Y[i + 1, j] - Y[i - 1, j]) / (2 * hx)) ** 2.0) ** 1.5
            )

            K2 = ((Y[i, j - 1] - 2 * Y[i, j] + Y[i, j + 1]) / hz**2.0) / (
                (1 + ((Y[i, j + 1] - Y[i, j - 1]) / (2 * hz)) ** 2.0) ** 1.5
            )
    ...
    return (
        (x[0] ** 2.0) * GK / SA
    )  # weighted average of gaussian curvature normalized to LOA^2
